...
// 命名派生：作品目录段与 store 文件名（store/work/{桶段}/{siteKey}_{siteWorkId派生段}/
// {role}_{seq 三位零填充}.{ext}）。派生规则（净化/截断/单射消歧、桶段哈希）由 SDK storepath
// 权威承载，本文件组装身份输入（领域行站点复合键）与库内布局前缀（store/work/，归
// storeRegistry 权威）。
...
// resolveStoreDir 解析作品落盘目录（relPath 域，正斜杠）：store/work/{桶段}/
// {siteKey}_{siteWorkId派生段}。桶段 = 复合键 SHA256 前 2 位 hex（256 桶，摊薄
// store/work 根目录扇出；同键恒同桶，路径锚定不变量保持）。
...
func (sess *execSession) resolveStoreDir(ctx context.Context) (string, error) {
	...
	return path.Join("store", "work", bucket, dirName), nil
}
