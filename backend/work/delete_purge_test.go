...
// purgeRsHardDeleter resource_store 级联删除替身（记录调用；purge 焦点在 persistent_store 行）
...
func newPurgeTestEnv(t *testing.T) (*Service, *persistentStore.Service, *gorm.DB) {
	...
	svc := NewService(
		...
		&purgeRsHardDeleter{},      // ResourceStoreHardDeleter
...
func TestDeleteWorkAndSurroundingDataPurgesStoreRows(t *testing.T) {
	...
	svc.resourceDeleter = &purgeResourceDeleter{resources: []*domain.Resource{resource}}
	...
	var workCount int64
	if err := db.Unscoped().Model(&domain.Work{}).Count(&workCount).Error; err != nil {
