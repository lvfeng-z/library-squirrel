...
- **站点作者信息刷新通知**：入库链 upsert 站点作者后经 `SiteAuthorRefreshScheduler`（authorInfo 提供，`SetSiteAuthorRefreshScheduler` 延迟注入）只传作者 DB ID 集合（含跨站引用作者，站点键由 authorInfo 按行 JOIN site 反查）；实现侧非阻塞（内部 goroutine），拉取编排与开关判定归 authorInfo。
