...
func newSaveWorkInfoTestEnv(t *testing.T) (*Service, *gorm.DB) {
	...
	svc := NewService(
		...
		tagNamespace.NewService(tagNamespace.NewRepository(db)), // TagNamespaceInventoryWriter（真实件）
		authorRole.NewService(authorRole.NewRepository(db)),     // AuthorRoleInventoryWriter（真实件）
	)
	return svc, db
}
