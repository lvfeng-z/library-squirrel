...
// 声明驱动为条目级：该（插件, 作品拉取条目）未声明 workSetRelationQuery 时跳过，不盲调 gRPC
// （SDK 侧类型断言仅作兜底）。
type workSetRelationFetcher struct {
	registry     *WorkFetchRegistry
	optionsQuery WorkFetchOptionQuerier
}

// NewWorkSetRelationFetcher 创建作品集父集关系获取器。optionsQuery 用于声明驱动（条目未声明方法组则不调用）。
func NewWorkSetRelationFetcher(registry *WorkFetchRegistry, optionsQuery WorkFetchOptionQuerier) *workSetRelationFetcher {
...
func (f *workSetRelationFetcher) QueryWorkSetRelations(ctx context.Context, pluginPublicId, extensionId string, siteId int64, siteWorkSetId string) ([]*pluginsdkdto.WorkSetRelationEntry, error) {
	...
	handler, err := f.registry.GetWorkFetcher(pluginPublicId, extensionId)
	if err != nil {
		return nil, fmt.Errorf("查找插件作品拉取扩展失败 %s/%s: %w", pluginPublicId, extensionId, err)
	}
	proxy, ok := handler.(*WorkFetchProxy)
	if !ok {
		return nil, fmt.Errorf("作品拉取扩展非 *WorkFetchProxy，无法查询父集关系")
	}
	return proxy.QueryWorkSetRelations(ctx, siteId, siteWorkSetId)
}

// declaresOption 查插件该作品拉取条目是否声明了指定可选方法组；未注入查询器时保守返回 true（向后兼容）。
func (f *workSetRelationFetcher) declaresOption(pluginPublicId, extensionId, option string) bool {
	if f.optionsQuery == nil {
		return true
	}
	return f.optionsQuery.HasWorkFetchOption(pluginPublicId, extensionId, option)
}
