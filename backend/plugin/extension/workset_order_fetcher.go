...
// 声明驱动为条目级：该（插件, 作品拉取条目）未声明 workOrderQuery 时跳过，不盲调 gRPC
// （SDK 侧类型断言仅作兜底）。
...
func NewWorkSetOrderFetcher(registry *WorkFetchRegistry, optionsQuery WorkFetchOptionQuerier) *workSetOrderFetcher {
...
func (f *workSetOrderFetcher) QueryWorkSetOrder(ctx context.Context, pluginPublicId, extensionId string, siteId int64, siteWorkSetId string) ([]*pluginsdkdto.WorkOrderEntry, error) {
	...
	if !ok {
		return nil, fmt.Errorf("作品拉取扩展非 *WorkFetchProxy，无法查询原站序")
	}
