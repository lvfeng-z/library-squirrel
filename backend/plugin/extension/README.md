...
| 组件 | 职责 |
| --- | --- |
| `loader.go` | 插件进程加载与 HostDeps 装配：契约版本校验（current 引用 SDK `transport.ContractVersion`，minSupported=11——siteAuthorFetch 数组化与运行时注册面声明化的破坏性分界）、声明面门控查询（清单原文校验 `ValidateManifestDeclarations`、能力集合派生 `deriveCapabilities`、拉取条目声明 `SiteAuthorFetchEntries`、条目级 `HasTaskHandlerOption`）、激活期按清单声明派生注册任务处理器/站点浏览器代理（元数据 name 取清单条目，停用/崩溃清理走 `UnloadPlugin` 的 `UnregisterAll`）、HostService RPC 桥接适配器（含库查询方法组 → `PluginContext` 的适配 `hostLibraryQueryProvider`） |
...
| `plugin_context.go` | 插件侧 `PluginContext` 实现：自存 KV、任务触发、前端通信转发、库查询方法组收口（每调用记诊断级日志——调用方插件 + 端点 + 关键参数）。**注册相关方法为契约 v11 前遗留**（`RegisterTaskHandler`/`RegisterSiteBrowser`/`UnregisterSiteBrowser` 已从 SDK 接口与 RPC 删除，宿主侧实现零调用方，待清死代码） |
...
| `registry.go` / `site_browser_registry.go` | 前端扩展注册中心（7 种 kind 统一管道）与 SiteBrowser/TaskHandler 复合键注册表（激活期按清单声明派生填充；URL 监听派生索引已迁 `backend/pluginTaskUrlListener`） |
