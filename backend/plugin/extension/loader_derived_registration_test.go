...
// 派生注册以其 TaskHandlers/SiteBrowsers 声明为输入
...
func TestRegisterDeclaredExtensionsBuildsProxies(t *testing.T) {
	...
	handlerExt, err := loader.taskHandlerRegistry.Get(info.PublicID, "main")
	if err != nil {
		t.Fatalf("任务处理器条目未注册: %v", err)
	}
	if handlerExt.Metadata.Type != model.ExtensionTypeTaskHandler ||
		...
		t.Errorf("任务处理器元数据 = %+v, 期望 name 取清单、归属字段取插件信息", handlerExt.Metadata)
	}
	if _, ok := handlerExt.Instance.(*TaskHandlerProxy); !ok {
		t.Errorf("任务处理器实例应为无状态代理, 实际 %T", handlerExt.Instance)
	}
...
func TestUnloadPluginClearsDerivedEntries(t *testing.T) {
	...
	if _, err := loader.taskHandlerRegistry.Get(info.PublicID, "main"); !errors.Is(err, ErrExtensionNotFound) {
		t.Errorf("停用后任务处理器条目应已注销, 实际 %v", err)
	}
