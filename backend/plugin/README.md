...
插件**生命周期管理**：从 ZIP 包安装、激活、卸载、重装插件，维护插件记录与运行状态。插件系统的架构（扩展点、SDK、Slot 通信、初始化时序）详见 `.claude/rules/plugin.md`，本文件只描述模块职责与对外接口。
...
- 依赖：`extension/`（扩展点加载）、插件 SDK（HostService 桥接）、settings（激活配置）
