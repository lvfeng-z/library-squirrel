...
- 被依赖：recycleBin（WorkSetRestorer）、work（入库链经 workSetWriterAdapter）、search（QueryRecycleWorkSetPage 读 work_set 已删行）、插件链（GetWorkSetBySiteWorkSetId → GetBySiteWorkSetIdAndSiteName，GORM 软删过滤下已删集判不存在走重建）、前端作品集页/回收站作品集 tab
...
- **既有 N+1 说明**：`QueryPageWithCover` 逐行查封面与 `ListWorkSetsByWorkId` 逐 ID 查集为存量形态（封面作品活性由 work 侧 ListByIds 软删过滤兜底、已删集由 GetById 过滤跳过），重构留待按需立项。
