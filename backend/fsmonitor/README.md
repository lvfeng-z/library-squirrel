...
- **watch 层排除暂存总根**（`NewPlatformDeps` 装配传 `staging.RootName`）：对新建目录的动态补挂（Stat→Walk→Add）存在瞬态打开窗口，与「先建临时名目录、随即 rename 定名」的操作（staging 作用域原子创建）竞态——Windows 上 rename 随机失败为 sharing violation（实测同构序列 300 次迭代 7 次）；暂存子树整体免挂后事件不再产生，事件处理层本就不消费该域。排除只作用于挂表层，其他目录的动态补挂照常（回归锚定 `source_watch_rename_test.go`）
