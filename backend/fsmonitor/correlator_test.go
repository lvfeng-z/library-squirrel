...
func TestProcessMove_TrackedFile(t *testing.T) {
	...
	sc := c.Process(context.Background(), FileChange{
		Kind: ChangeMove, Path: "store/work/A/旧.jpg", ToPath: "store/work/B/新.jpg", DetectedAt: 100,
	})
	if sc == nil {
	...
	if sc.FromPath != "store/work/A/旧.jpg" || sc.ToPath != "store/work/B/新.jpg" {
...
func TestProcessMove_UntrackedFile(t *testing.T) {
	...
	sc := c.Process(context.Background(), FileChange{
		Kind: ChangeMove, Path: "store/work/外部/x.jpg", ToPath: "store/work/外部/y.jpg",
	})
	if sc != nil {
...
func TestProcessMove_Directory(t *testing.T) {
	...
	store := map[string]StoreRecord{
		"store/work/旧目录": {ID: 1, FilePath: "store/work/旧目录"},
	}
	c := newTestCorrelator(store)
	sc := c.Process(context.Background(), FileChange{
		Kind: ChangeMove, IsDir: true, Path: "store/work/旧目录", ToPath: "store/work/新目录",
	})
	if sc != nil {
...
func TestProcessCreate_Regression(t *testing.T) {
	store := map[string]StoreRecord{
		"store/work/old.jpg": {ID: 7, FilePath: "store/work/old.jpg", ContentFingerprint: "fp"},
	}
	...
	sc := c.Process(context.Background(), FileChange{
		Kind: ChangeCreate, Path: "store/work/new.jpg", DetectedAt: 5,
	})
...
func TestProcessRemove_Regression(t *testing.T) {
	store := map[string]StoreRecord{
		"store/work/gone.jpg": {ID: 9, FilePath: "store/work/gone.jpg"},
	}
	c := newTestCorrelator(store)
	sc := c.Process(context.Background(), FileChange{
		Kind: ChangeRemove, Path: "store/work/gone.jpg", DetectedAt: 8,
	})
