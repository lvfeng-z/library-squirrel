...
func newTestSetup(t *testing.T) (ManifestIngestor, *gorm.DB, *persistentStore.Service, string) {
	...
	ing := NewIngestor(NewRepository(db), &testTransactor{db: db}, psService)
...
func TestIngestRoundTripThenIdempotent(t *testing.T) {
	...
	r1, err := ing.Ingest(ctx, manifest, mapFileSource(files))
	if err != nil {
	...
	r2, err := ing.Ingest(ctx, manifest, mapFileSource(files))
	if err != nil {
...
func TestIngestSchemaVersionRejected(t *testing.T) {
	...
	_, err := ing.Ingest(context.Background(), manifest, mapFileSource(files))
...
func TestHandlerImportFromZip(t *testing.T) {
	...
	handler := NewHandler(NewIngestor(NewRepository(db), &testTransactor{db: db},
