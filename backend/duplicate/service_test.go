...
// fakeRepo 查重仓储桩:站点名→站点、站点作品定位均可预置,并支持查询失败模拟
type fakeRepo struct {
	sitesByName map[string]*entity.Site            // 站点名 → 站点行
	worksBySite map[int64]map[string]*entity.Work  // 本库站点 ID → 站点作品 ID → 作品行
	siteErr     error
	workErr     error
}

func (f *fakeRepo) ListSitesByNames(ctx context.Context, names []string) ([]*entity.Site, error) {
	if f.siteErr != nil {
		return nil, f.siteErr
	}
	var out []*entity.Site
	for _, n := range names {
		if s, ok := f.sitesByName[n]; ok {
			out = append(out, s)
		}
	}
	return out, nil
...
func mkSite(id int64, name string) *entity.Site {
	s := entity.NewSite()
	s.ID = id
...
// stdEnv 标准环境:站点 "site-a"→id=10 挂作品 "w1"→id=500(角色 image+thumbnail)
func stdEnv() (*fakeRepo, *fakeRoleSets) {
	repo := &fakeRepo{
		sitesByName: map[string]*entity.Site{"site-a": mkSite(10, "site-a")},
...
func TestCheck_Miss(t *testing.T) {
	t.Run("站点不存在_未命中", func(t *testing.T) {
		...
		res := checkOne(t, svc, DuplicateCheckItem{SiteName: "unknown-site", SiteWorkID: "w1", Roles: []string{entity.StoreTypeImage}})
	...
	t.Run("站点存在_作品未收录_未命中", func(t *testing.T) {
		repo, roleSets := stdEnv()
		svc := NewService(repo, roleSets)
		res := checkOne(t, svc, DuplicateCheckItem{SiteName: "site-a", SiteWorkID: "not-exist", Roles: []string{entity.StoreTypeImage}})
		if res.Class != DuplicateMiss {
			t.Fatalf("期望未命中, 实际 class=%d", res.Class)
		}
	})
	t.Run("空站点名_未命中", func(t *testing.T) {
		repo, roleSets := stdEnv()
		svc := NewService(repo, roleSets)
		res := checkOne(t, svc, DuplicateCheckItem{SiteName: "", SiteWorkID: "w1", Roles: nil})
		if res.Class != DuplicateMiss {
			t.Fatalf("空站点名期望未命中, 实际 class=%d", res.Class)
		}
	})
}
...
func TestCheck_HitNoConflict(t *testing.T) {
	t.Run("显式板块_交集空_命中无冲突_保留作品ID", func(t *testing.T) {
		repo, roleSets := stdEnv()
		svc := NewService(repo, roleSets)
		res := checkOne(t, svc, DuplicateCheckItem{SiteName: "site-a", SiteWorkID: "w1", Roles: []string{entity.StoreTypeVideoTrack, entity.StoreTypeAudioTrack}})
	...
	t.Run("已有零行_命中无冲突", func(t *testing.T) {
		...
		svc := NewService(repo, roleSets)
		res := checkOne(t, svc, DuplicateCheckItem{SiteName: "site-a", SiteWorkID: "w1", Roles: []string{entity.StoreTypeImage}})
	...
	t.Run("板块空_已有零行_命中无冲突", func(t *testing.T) {
		...
		roleSets.sets[500] = map[string]struct{}{}
		svc := NewService(repo, roleSets)
		res := checkOne(t, svc, DuplicateCheckItem{SiteName: "site-a", SiteWorkID: "w1", Roles: nil})
...
func TestCheck_HitConflict(t *testing.T) {
	t.Run("显式板块_交集非空_冲突且载荷为交集原序", func(t *testing.T) {
		repo, roleSets := stdEnv()
		svc := NewService(repo, roleSets)
		res := checkOne(t, svc, DuplicateCheckItem{SiteName: "site-a", SiteWorkID: "w1", Roles: []string{entity.StoreTypeVideoTrack, entity.StoreTypeThumbnail, entity.StoreTypeImage}})
...
func TestCheck_QueryFailure(t *testing.T) {
	t.Run("站点查询失败_返回error", func(t *testing.T) {
		...
		svc := NewService(repo, roleSets)
		if _, err := svc.Check(context.Background(), []DuplicateCheckItem{{SiteName: "site-a", SiteWorkID: "w1"}}); err == nil {
...
func TestCheck_IndexAligned(t *testing.T) {
	repo := &fakeRepo{
		sitesByName: map[string]*entity.Site{
			"site-a": mkSite(10, "site-a"),
			"site-b": mkSite(20, "site-b"),
		},
		worksBySite: map[int64]map[string]*entity.Work{
	...
	items := []DuplicateCheckItem{
		{SiteName: "site-a", SiteWorkID: "w1", Roles: []string{entity.StoreTypeVideoTrack}}, // 交集空 → 命中无冲突
		{SiteName: "site-b", SiteWorkID: "w9", Roles: []string{entity.StoreTypeAudioTrack}}, // 交集非空 → 命中冲突
		{SiteName: "site-a", SiteWorkID: "nope", Roles: []string{entity.StoreTypeImage}},    // 未命中
	}
