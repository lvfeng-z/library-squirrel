...
| `ShareReceive(link, password)` | 接收分享：解析链接 → 同步预拉清单 → 建任务树（父容器 + 每作品一子任务）→ 整树启动，返回 `ShareReceiveResult{parentTaskId, workCount, workNames}`（作品名净化后、与子任务命名一致；进度/终态由任务面板树形承载）。link 接受深链 `library-squirrel://share/{中继}/{token}#k={密钥}` 与落地页链接 `https://{中继}/s/{token}#k={密钥}`；password 为设有访问密码时的明文（落载荷只有摘要）。清单拉取失败不建任何任务、返回用户可读文案 |
