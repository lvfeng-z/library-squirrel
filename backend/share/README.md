...
- **收件拨号统筹**（`DialCoordinator`，`dial_coordinator.go`）：接收方进程内统一仲裁 share-receive 各子任务文件拉取拨号的**纯并发/速率能力**（不感知业务实体），双门控——**并发流槽**（默认 8，对齐发送方会话 8 流上限，并发活跃流 ≤8 即不被发送方拒绝）+ **拨号速率**（默认 50/min，低于中继每 IP 每分钟 60 限流并留余量，不触发 rate_limited）。进程级单例由 app.go 装配 `share.NewDialCoordinator(8, 50)`，经 `Service.SetDialCoordinator`（`service.go:166`）写入 `sessionRuntimeOptions.dialCoordinator` 注入接收链；`newReceiveClient` 未收到注入时回落包级默认单例 `DefaultDialCoordinator`（`dial_coordinator.go:41`），装配遗漏下门控仍生效。接入形态：`receiveClient.fetch`（`receive_client.go:154`）拨号前先 `TakeDial`（速率令牌）再 `AcquireSlot`（并发流槽），槽覆盖整条流下载期、随内容读取器 `Close` 归还（`receive_client.go:326`；拨号失败未开流即时归还，`receive_client.go:163`）；manifest 预拉（`fetchManifest`，`receive.go:203`）与文件拉取（`stageFile`，`task_execution.go:859`）同走 fetch 一处接入全链生效。
...
- **manifest 携带双哈希**：分享宿主会话开始时（收集→规划后、Serialize 前）对每个非 Missing 文件**一次读流**预填 `ContentFingerprint`（size + 头部 64KB SHA256，`<size>:<hex>`，与库内 `persistent_store.content_fingerprint` 同口径）+ `Sha256`（全量 hex）——`Sha256` 即「文件期望哈希」预下载参考：收件方下载前逐文件判定「本地已有文件是否通过校验」跳过拉取（内容未变重跑零网络），Ingest 落盘边写边算校验下载完整性（分享下载校验随本路径免费解锁）。代价是发布时对每个文件算全量哈希（大库发布前全盘读 IO，落在「分享进行中」阶段，L2 3GB ≈ 6-10s）；传输完整性仍由每记录 GCM 认证标签 + 应答头 size 字节计数守门。
