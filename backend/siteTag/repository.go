...
func (r *SiteTagRepository) BatchUpsert(ctx context.Context, tags []*entity2.SiteTag) error {
	...
	return r.dbFromCtx(ctx).WithContext(ctx).Clauses(clause.OnConflict{
		...
		// upsert 仅更新插件权威字段：site_tag_name/description。local_tag_id（用户手动 site→local 桥接）、
		// base_site_tag_id、last_use 不归插件管：它们不在插件 DTO 中，excluded（待插入新行）为零值 NULL，
		// 列入 DoUpdates 会用 NULL 覆盖已有值
