...
// TestUpsertConflictColumnSet 键冲突时列集语义：插件权威列（site_tag_name/description）
// 按 excluded（待插入行）刷新；用户策展列（local_tag_id 桥接/base_site_tag_id/last_use）
// 不在更新集，保持既有值。BatchUpsert 与 Upsert 共用同一冲突更新列集，两法一并锚定
func TestUpsertConflictColumnSet(t *testing.T) {
	...
	if got2.SiteTagName.String != "名v3" {
		t.Fatalf("Upsert 名应刷新，实际 %q", got2.SiteTagName.String)
	}
