...
## 一句话职责
把用户选中的作品/作品集（含成员作品递归闭包、标签/作者/作品集/文件等全部关联）收集为可移植的导出数据模型（manifest 契约 + 源文件清单），并按确定性命名的作品目录结构打包为 ZIP（异步执行 + 进度事件 + 可取消），是导出功能的**数据面 + 执行面**（方案见 `../library-squirrel-docs/plan/导出功能总体方案.md`）。
