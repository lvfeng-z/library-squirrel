...
| `CreateTaskByURL(req)` | URL → 路由到处理该 URL 的插件扩展点 → 创建任务。`CreateTaskByURLRequest{URL, ChosenPluginPublicId, ChosenExtensionId, Remember}`：两显选键为空即未显选（程序化触发恒为此态）；交互触发命中多候选且未显选时先查消歧记忆，命中即静默直路由，未命中返回冲突载荷 `Conflict=true` + `ConflictCandidates`（未调用任何插件），前端选择后带键重发；`Remember` 为显选附带「记住此选择」勾选态，显选且路由成功后落粘性记忆 |
...
- **两入口分工**：`CreateTaskByURL`（程序化面）恒按候选全键字典序、不问冲突也不触达记忆——宿主 `CreateTask`、插件经宿主建任务走此入口；`CreateTaskByURLWithChoice`（交互面，前端手动建任务，Handler 暴露的即此）带显选键时该扩展点候选经排序键（置首前缀 "0"）排在其他候选之前，未带显选键且候选 ≥2 时先查消歧记忆（命中且记住的候选仍在候选集内 → 等价显选静默直路由；未命中或候选不在场 → 返回冲突载荷（`Conflict=true` + `ConflictCandidates`，按候选全键字典序、首位即默认选中项）且**不调用任何插件**，由前端选择后带键重发）。显选键经**两键联合校验**（插件 ID + 扩展点 ID 同时候选集命中），未命中报 `ErrChosenCandidateInvalid`。
- **消歧记忆（交互面专用）**：多候选冲突的显选可被记住，同站点域同候选组合的冲突复现时免问直路由。记忆键 = （记忆域, 站点域, 候选组合）：站点域由候选条目声明的 siteKey 全体一致时取声明值、否则退任务 URL host 兜底（host 解析失败本轮不查不写）；候选组合 = 按全键字典序排序的候选全键列表（新候选加入即组合变化、自动重问）；值为显选候选全键。键与值的构造/拆解统一走 `stickymemory` 包的编码函数。写入门控三条件齐备：显选有效 + `Remember` 勾选 + 本次路由成功（失败的选择不入记忆）；命中直路由不重写。记忆读写仅存在于交互入口分支内——程序化入口不查不写由代码位置机械保证。
...
- 依赖：URL 监听派生索引（`urlListener.ListListener`，派生源为插件清单 `extensions.workFetch[].urlPatterns`，宿主激活期按清单条目派生登记、条目级粒度＝插件公开 ID + 条目 id）、多候选路由基座（`backend/route`，URL 监听器候选经其按全键字典序逐个尝试）、粘性记忆（`DisambiguationMemory` 窄接口，`stickymemory.Service` 实现，app.go 装配；交互面冲突的免问直路由与显选落记忆）、站点 / 作品集查询、事务执行器（Transactor，删除链编排用）、workDir 读取（`workDirGetter func() string`，删除链清下载暂存目录用）、resource.task_id 引用清理（repository 层原生 UPDATE，删任务前置义务）
