...
- 依赖：URL 监听器（`urlListener.ListListener`，由插件提供）、多候选路由基座（`backend/route`，URL 监听器候选经其按全键字典序逐个尝试）、站点 / 作品集查询、事务执行器（Transactor，删除链编排用）、workDir 读取（`workDirGetter func() string`，删除链清下载暂存目录用）、resource.task_id 引用清理（repository 层原生 UPDATE，删任务前置义务）
