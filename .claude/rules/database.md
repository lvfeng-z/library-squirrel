...
| 表 | 字段 | 根目录 | 说明 | 存储示例 |
|---|---|---|---|---|
| `persistent_store` | `file_path` | workDir | 资源文件存储 | `store/work/73/pixiv_128937464/image_000.jpg`、`store/work/16/bilibili_BV1xx411c7mD_4538792/videoTrack_000.mp4` |
| `backup` | `file_path` | workDir | 备份文件路径 | `backup/2026/06/08/文件.mp4` |

> 命名规约:所有插件 store(含 thumbnail)与主程序派生 store(videoMain)统一进 `store/work/{桶段}/{site_key}_{siteWorkId 派生段}/`,文件名恒为 `{role}_{seq 三位}.{ext}`(身份键纯函数派生,SDK `storepath` 包,桶段=复合键 SHA256 前 2 位 hex 摊薄根目录扇出,目录段单射消歧——净化变更追加哈希段、超长截断),详见 `doc/store-naming-convention.md`。历史独立目录 `store/thumbnail/` 白名单条目已退役(零写入方、库内零存量行)。
...
- **声明登记面**：`backend/migration/foreign_keys.go` 的 `fkBatches`（全库 31 对）。SQLite 无 `ALTER TABLE ADD CONSTRAINT`，存量表挂 FK 经**表重建舞步**（以 `sqlite_master` 现表 DDL 为源文本注入 FK 子句→建新表→拷数据→删旧表→改名→复原索引；**禁用实体重建 DDL**——实体字段序与表列序漂移会静默错位）。幂等标记：`pragma_foreign_key_list` 的（引用列, 引用表）对（SQLite FK 无独立命名）。
