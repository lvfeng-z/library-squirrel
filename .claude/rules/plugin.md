...
| 落盘路径派生 | `storepath` 包（`WorkDirName`/`StoreFileName`，本地纯函数零依赖） | 库内最终落盘路径（`store/resource/{site_key}_{siteWorkId 派生段}/{role}_{seq 三位}.{ext}`）由插件据任务身份与 specs 顺序本地推导（顺序确定性契约见 `doc/plugin-dev-guide.md` 6.1），不经宿主 RPC 查询 |
