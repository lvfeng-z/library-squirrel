...
- **ROW_SHAPE_DATAKEY_SYNC** (P1): 表格行的数据形态变更（改用组合/嵌套 DTO、字段下沉一层）时，`data-key` 必须与 thead 列键**同批同步**，且该表的行操作链（查看/编辑/删除按钮、行内选择、批量入口）必须经**真实点击**验证——`data-key` 是行唯一标识的取值路径，`DataTable`/`SearchTable` 一律经 `getPropByPath(row, dataKey)` 取件（`DataTable.vue:108`），取空即抛「无法获取行数据的唯一标识」致该行全部操作失效。**类型检查兜不住**：`dataKey` 在 `DataTable.vue:20` 声明为裸 `string`，与行类型无关联，`vue-tsc` 对 `'id'` 与 `'author.id'` 一视同仁。同族核对面：行内其他直读行字段的访问点（选中项、编辑保存、对话框回查）。加固方向（未实施）：把 `DataTable`/`SearchTable` 的 `dataKey` 收窄为 `keyof T | string` 让编译期兜底（先例：本地作者页数据源切 `LocalAuthorFullDTO` 后 thead 键已全改 `author.*`、唯 `data-key` 漏改，`vue-tsc` 零错却在运行期点行按钮时暴露）。
