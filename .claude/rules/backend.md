...
- **Site（站点）**: 远程来源（pixiv 等）。唯一身份 = `site_key`（SDK `identity` 注册表分配的品牌 slug，如 "pixiv"，not null + 唯一索引；键一经发布不可变、注册表只增不改，未注册键在导入/分享接收被拒绝；站点表 = 注册表投影而非本库使用面，站点行写入方封闭为「启动投影同步」单一（`SyncFromRegistry`，insert-only，缺失键落注册表权威值、既有行不动），无删除语义——投影行删除后下次启动同步复活；导入/分享回灌按键 find-only 直查（注册键行缺失属数据库异常，显式报错）；键=身份、名=展示，两列职责分离禁止合并回按名匹配）；`site_name` 纯展示（非唯一，建行取注册表权威名，用户可改且编辑持久）。跨库匹配/查重/关联一律按键，作品复合键 = (site_key, site_work_id)。规范见 `doc/site-identity-spec.md`
...
- **namespace（tag 关联级维度）**: tag 关联（`re_work_tag`）上的 namespace 维度，**非 tag 身份的一部分**——tag 实体（local_tag/site_tag）身份扁平、无 namespace 列。`re_work_tag.namespace` 为 `string not null default ''`（空串=无 ns），两条唯一索引 `(work_id, local_tag_id, namespace)` / `(work_id, site_tag_id, namespace)` 含 ns——同作品同标签多 ns 可达（e-hentai `female:tagA`+`male:tagA` 落一行 site_tag + 两条关联）。开放字符串 + 清单表 `tag_namespace` 承接候选展示：`value` 唯一 / `label` 显示名 / `origin` 三态 builtin>user>plugin（数值即优先序，升级不降级、内置行不可改写、`last_use` 照常刷）+ 启动投影 insert-only（内置集 7 项 language/character/parody/female/male/misc/general；artist 归 author 体系不入 namespace）；清单 find-or-create 钩子与关联写入同事务（dbFromCtx），维度值写入前归一化（TrimSpace + 小写折叠）。SITE 轨开放用户自设：插件声明 ns 直写 PLUGIN 关联行（重拉窄域重建按声明刷新），用户手动挂联的 MANUAL 行 ns=用户值、重拉不动。搜索支持 per-namespace 过滤（不设 ns 即不过滤）。设计见 `../library-squirrel-docs/plan/关联级维度体系重构方案.md`
- **author role（作者关联级维度，与 namespace 同构）**: `re_work_author.role_name`——同形态（`string not null default ''` 空串=无 role、两条唯一索引含 role_name、同作品同作者多 role 可达、清单表 `author_role` 同构且内置集为空靠三源自然生长、归一化与同事务钩子同款）。声明面 = SDK `TaskSiteAuthorDTO.RoleName`（插件入库链直写 PLUGIN 关联行）；返回面 = 库查询 `ListAuthorsByWorkId` 返关联条目 `WorkLocalAuthorEntry`/`WorkSiteAuthorEntry`（`author` + `role_name`，不给作者实体级 DTO 加 role）。设计见同上方案
- **Task（任务）**: 作品创建流程（URL → 插件 → 保存），插件 Create 声明 ResourceType
- **WorkDir（工作目录）**: 用户配置的资源库根目录（settings 模块管理，`GetWorkDir` 空串=未配置）。**未配置是合法显式状态**：读取侧判空串不 Trim、保存侧 Trim 落库；依赖模块自行处理该状态——启动期服务（fsmonitor 监控/对账、recycleBin 清扫）空串不启动，请求期入口（落盘/还原/导出/打开目录等）经 `settings.RefuseIfUnconfigured` 返回哨兵错误并发射 `workdir:unconfigured`（事件统一、既有守卫的领域文案不统一）；禁止把空串当合法路径拼接文件系统操作（会被静默相对化为进程工作目录）。设计见 `../library-squirrel-docs/plan/工作目录未配置态显式化方案.md`
...
- **STAGING_SCOPE_GOVERNANCE** (P1): 暂存域（`{workDir}/staging/`）作用域唯一创建路径 = `staging.CreateScope`/`CreateExportScope` 原子入口（临时名目录写好 scope.json 再 rename），禁止消费方自建目录；属主根与回收策略分型集中登记在 staging 包根注册表（download/share-receive 谓词判活、import/merge 启动清空、export 账本驱动），新增属主只在该表登记，禁止另立暂存根或私行清扫；启动清扫 `SweepAtStartup` 先于任何能创建作用域的服务（app.go 装配保证）；暂存总根不在 store 白名单与 backup 域，fsmonitor 零感知——零感知是两道：事件处理层不消费 + **watch 层整体免挂**（`NewPlatformDeps` 装配时排除；动态补 watch 的瞬态打开窗口与作用域「临时目录→rename 定名」竞态，Windows 上 rename 随机撞 sharing violation 致任务失败，只做事件层过滤挡不住句柄竞态）；作用域路径属 absPath 域仅 os.* 调用点消费，入 DB 的暂存相对路径（如收件 `manifest_path`）为 relPath 域正斜杠。布局与回收规约单一来源：`backend/staging/README.md`。
...
- **ROUTE_SEMANTICS_SINGLE_SOURCE** (P0): 触发式功能的多候选路由**四不变量**（候选排序 / 逐候选单射尝试 / 失败二分分发 / 收口两态分报）只允许在 `backend/route` 单点实现，各消费面经 `Adapter` 接入、只自定「候选发现 + 调用 + 三态分类」两维；**禁止各消费面各自实现同一语义**（反例=该语义漂移的历史成因：任务创建跨 pattern 尝试序取自 Go map 迭代而随机、作者拉取把「零候选」与「全不适配」合并成一条文案）。基座**不感知**显选、冲突与任何协议/业务概念——显选置首由适配器折进 `OrderKey`（基座对候选整体做稳定排序，适配器给的切片序必被覆盖），「哪些错误算明示不归属」属协议知识留适配器分类。新增消费面接入前先读 `backend/route/README.md` 的边界护栏。
- **INGEST_TX_ATOMICITY** (P0): persistent_store 行的入库唯一形态=提交点四调用（`PrepareIngest` 登记 → `PlaceIngest` 落位 → 调用方业务事务内 `CommitIngest` 建行并删登记行 → 异常 `AbortIngest` 按登记声明撤回）；「事务外直接写文件+建行」的非事务入库 API 不得复活。登记行删除与建行**同事务**（判据精确性：两行不可能一存在一不存在，崩溃后凭登记账本 `store_ingest_journal` 精确收口）；撤回处置（退回暂存/丢弃）由调用方登记时**声明、必填无默认**（该轨能否续传、是否可重产只有调用方知道，能力包只执行不推断）；启动恢复 `RecoverIngest` 在 app 启动序列先于 fsmonitor 执行。机制见 `backend/persistentStore/README.md`「入库事务机制」。
