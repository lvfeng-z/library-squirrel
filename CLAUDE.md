...
- **config/settings.json**: 运行时用户设置（通过 SettingsHandler 读写），含工作目录、下载/作品/插件/回收站设置、向导完成状态、外观主题（`appearance.theme`，由前端 `useThemeStore` 管理）、主页多选开关（`appearance.multiSelectEnabled`，由前端 `MainView` 读写，默认关）等
...
- 已覆盖：`taskManager`、`task`、`work`、`workSet`、`resource`、`plugin`、`backup`、`persistentStore`、`reWorkAuthor`、`reWorkTag`、`recycleBin`、`fsmonitor`、`backupGovernance`、`workdirGuard`、`export`、`import`、`share`
