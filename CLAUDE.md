...
- **config/settings.json**: 运行时用户设置（通过 SettingsHandler 读写），含工作目录、下载/作品/插件/回收站设置、向导完成状态、外观主题（`appearance.theme`，由前端 `useThemeStore` 管理）等。主页多选开关为会话内开关（不持久化，默认关；多选态点图即勾选）——旧字段 `appearance.multiSelectEnabled` 已休眠不再读写
...
- **读文件纪律**: 大文件（>500 行或 >30KB）禁止整 Read——先 Grep 定位目标行，再用 `offset`/`limit` 读片段；优先读模块 `README.md` 定位，再决定是否碰大文件。
- **术语与标识符纪律**（写给用户读的文本：决策提问、方案、任务树、汇报）: 判据 = 词有无字面义——**有字面义的通用词**（「顺延」「收口」「显选」「拒载」类日常词）**禁止挪用为项目专用语义**（读者照字面理解会得到错的机制；要表达该机制直接把机制描述出来）；**无字面义的独有词**（标识符、契约字段名）首现带半句说明。同一交付内反复指代可用**局部短称**（首现声明「下称 X」，不跨文档沉淀）。引用模块方法/变量时：仅作指代工具的少用符号、改写「它做什么 + `file:line`」；本身是讨论对象（契约字段名等）的必须写并首现说明；名实易混的点明「不是〈字面义〉」。**不建集中术语表**（`doc/ai-assistant/glossary.md` 已废除，勿重建）。
...
- **交付单元含私有文档库 `../library-squirrel-docs`**：凡本会话在该库新建或修改的文件（方案、缺陷记录、待办、任务树 `TREE.md` 的节点状态与外链同步等），须在**同一会话内**提交到该库——禁止跨会话把文档积压在工作区；提交纪律与主仓同（显式路径暂存、禁 `git add -A`、中文规范）。多仓提交顺序 = SDK → 主仓 → 插件仓 → 文档库
- 任务树与其指向的文档同批提交：`TREE.md` 的节点状态/外链变更，不得与该变更引用的方案或报告分属不同提交周期
...
- 已覆盖：`taskManager`、`task`、`download`、`work`、`workSet`、`resource`、`plugin`、`backup`、`persistentStore`、`storeRegistry`、`reWorkAuthor`、`reWorkTag`、`recycleBin`、`fsmonitor`、`backupGovernance`、`workdirGuard`、`export`、`import`、`share`、`settings`、`authorInfo`、`route`
...
| 场景 | 技能 |
| --- | --- |
| 用户描述 bug / 贴报错 / 要求排查异常行为 | `diagnose` |
| 要求提交、任务收尾提交（含多仓按序） | `commit` |
| dev 实机手测 / 验证功能在真实应用中的表现 | `live-test` |
| 执行含「阶段清单」的计划 | `phase-executor` |
| 多层派生的基建/修复/重构 | `task-graph` |
| 新需求或方案设计 / 方案审查 | `requirements-analyst` / `plan-reviewer` |

**纪律**：场景命中即先调技能，**不得以「任务小 / 我已有强假设 / 代码是我写的 / 结论显而易见」为由跳过**；技能内若需轻量路径，由技能自身声明，不由调用方临时降级。下表各技能的场景细节见其后条目。
...
- **向用户提出决策时的呈现契约**：每条决策给足七件——①一句领域语言说明在决定什么（禁以符号起头）②现状与前提（判断所依赖的事实先查证写明、**先讲清前提再抛选项**，不留作用户作业）③相关文档入口（给可读路径；该处无文档时明说并贴关键片段）④选项对照（每项「改前→改后 + **代价**」——判断依据是代价不是收益）⑤不做会怎样 ⑥推荐 + 回退成本 ⑦术语与标识符纪律（见「编码规则（全局）· 通用」）。**反向纪律**：说不出「用户以后会在哪里看到它的影响」的决策不问用户——按项目既有原则自决后告知。
