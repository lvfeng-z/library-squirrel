...
func (l *Loader) registerExtensions(entry ExtensionPoint, pluginInfo *PluginInfo) error {
	...
	case SlotExtension:
		...
		domainSlot := domain.NewSlotConfig()
		...
		domainSlot.SlotType = domain.SlotType(slotConfig.SlotType)
		domainSlot.Content = slotConfig.Content
		domainSlot.ContentType = domain.ContentType(slotConfig.ContentType)
...
func (l *Loader) GetTaskHandler(pluginPublicId, contributionId string) (domain.TaskHandler, error) {
...
type TaskHandlerExtension interface {
	ExtensionPoint
	GetTaskHandler() domain.TaskHandler
}

// SiteBrowserExtension 站点浏览器扩展点接口
type SiteBrowserExtension interface {
	ExtensionPoint
	GetSiteBrowser() domain.SiteBrowser
}
