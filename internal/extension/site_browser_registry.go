...
type SiteBrowserRegistry struct {
	mu         sync.RWMutex
	extensions map[string]*model.Extension[domain.SiteBrowser] // key: pluginPublicId/extensionId
}

// NewSiteBrowserRegistry 创建站点浏览器注册中心
func NewSiteBrowserRegistry() *SiteBrowserRegistry {
	return &SiteBrowserRegistry{
		extensions: make(map[string]*model.Extension[domain.SiteBrowser]),
	}
}

// Register 注册扩展点
func (r *SiteBrowserRegistry) Register(extension *model.Extension[domain.SiteBrowser]) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
// Get 获取扩展点
func (r *SiteBrowserRegistry) Get(pluginPublicId string, extensionId string) (*model.Extension[domain.SiteBrowser], error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *SiteBrowserRegistry) GetByPlugin(pluginPublicId string) ([]*model.Extension[domain.SiteBrowser], error) {
	r.mu.RLock()
	defer r.mu.RUnlock()

	var result []*model.Extension[domain.SiteBrowser]
	prefix := pluginPublicId + "/"
...
// List 列出所有扩展点
func (r *SiteBrowserRegistry) List() []*model.Extension[domain.SiteBrowser] {
	r.mu.RLock()
	defer r.mu.RUnlock()

	result := make([]*model.Extension[domain.SiteBrowser], 0, len(r.extensions))
	for _, ext := range r.extensions {
		result = append(result, ext)
	}
	return result
