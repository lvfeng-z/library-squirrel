...
- 1000ms 聚合窗内按（**category + level + title**）合并为一张卡片。
...
- **全量快照**（后端防抖推完整状态）→ 消费端必须做**迁移检测**：对比替换前状态，活跃→活跃原地 `update`；活跃→终态转终态 + `announce` + 脱离；未见过的终态不打扰。参考实现：`UseTaskStore.loadSnapshot`。
