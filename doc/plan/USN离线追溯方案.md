...
| **C-4**（✅ 完成 2026-08-13） | `usnProvider` 实现 `OfflineChangeProvider`，`NewPlatformDeps` windows 注入 — `usn_provider_windows.go`：FSCTL 常量（QUERY 0x000900F4 METHOD_BUFFERED / READ_USN_JOURNAL 0x000900BB **METHOD_NEITHER**）+ `usnJournalData`/`readUSNJournalDataV1`（**V1 48B，V0 触发 ERROR_INVALID_USER_BUFFER**）+ `ErrCursorInvalid`；`ChangesSince`（openVolume→QUERY→CursorStore 读游标→R3 失效检测→D5 首次→懒构建 FRN 缓存→readJournalBatch 64KB 堆缓冲批读→pairAndResolve→存游标）；`pairAndResolve`（rename 按 FRN 整批配对+缓存增量维护+白名单过滤，目录改名发 ChangeCreate(IsDir) 走 processDirCreate）。**provider 持有 CursorStore**（修 C-3 时序：cursorStore 改为 NewPlatformDeps 入参；移除 Deps.CursorStore 字段）；非 Windows 桩 `usn_provider_other.go`。7 单测 + 管理员集成测试通过。 | C-1/2/3 |
...
| **C-6**（✅ 完成 2026-08-13） | `runOfflineReconcile` 编排分叉 + USN/对账去重 + 游标失效检测 — `service.go`：USN 段（OfflineProvider!=nil 时 ChangesSince→逐条 Correlator.Process→dispatch+记 dedupKey）；err（含 ErrCursorInvalid）降级；对账段始终再跑、dispatch 前按 `{Kind,FromPath,ToPath,StoreID}` 去重跳过 USN 已报（D7）。D6 漏报（存游标早于 dispatch）由 D2 对账兜底。日志 USN N 条/对账补 M/去重 D。 | C-4/5 |
